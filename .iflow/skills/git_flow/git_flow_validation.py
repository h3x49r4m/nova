...
class GitFlowValidation:
    ...
    def validate_before_operation(self, operation: str) -> tuple[bool, str]:
        ...
        try:
            ...
            elif operation == 'review' and not self.config.workflow_state:
    ...
    def _is_git_repository(self) -> bool:
        ...
        try:
            code, _stdout, _stderr = run_git_command(
                ['rev-parse', '--git-dir'],
                cwd=self.repo_root
            )
            return code == 0
    ...
    def get_validation_report(self) -> dict[str, Any]:
        ...
        for _branch_name, branch_state in self.config.branch_states.items():
            is_valid, msg = validate_branch_state(branch_state.to_dict())
            if is_valid:
