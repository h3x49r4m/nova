...
class GitFlowCheckpoint:
        ...
        try:
            ...
            for cp in checkpoints:
                if cp['age_days'] > max_age_days or len(checkpoints) - len(deleted) > max_count:
                    code, _msg = self.delete_checkpoint(cp['id'])
                    if code == 0:
    ...
    def _get_oldest_checkpoint(self, checkpoints: list[Path]) -> str | None:
    ...
    def _get_newest_checkpoint(self, checkpoints: list[Path]) -> str | None:
