...
class PipelineVersionManager:
    ...
    def check_updates(self) -> tuple[bool, str | None]:
        ...
        if 'nested' in schema:
            for nested_field, nested_schema in schema['nested'].items():
                if nested_field in state and isinstance(state[nested_field], dict):
                    _nested_valid, nested_errors = self.validate(state[nested_field], nested_schema)
...
class BackupManager:
    ...
    def create_backup(self, state: dict, metadata: dict | None = None) -> str:
    ...
    def restore_backup(self, backup_id: str) -> dict | None:
...
class PipelineUpdateManager:
    ...
    def rollback_to_version(self, target_version: str, state: dict, backup_id: str | None = None) -> tuple[bool, str]:
        ...
        try:
            self.version_manager.get_rollback_path(target_version)
        except ValueError as e:
            return False, str(e)
