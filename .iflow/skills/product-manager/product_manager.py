...
class ProductManager:
    ...
    def prioritize_features_rice(
        self,
        features: list[dict[str, Any]],
        rice_scores: dict[str, dict[str, float]] | None = None
    ) -> list[dict[str, Any]]:
        """
    ...
    def update_project_spec(
        ...
        try:
            ...
            for idx, story in enumerate(user_stories, 1):
                ...
                us_section += "  - Acceptance Criteria:\n"
...
- TDD: compliant"""
            ...
            error_msg = f"Git error during commit: {e}"
            self.logger.error(error_msg)
...
def main():
    ...
    subparsers.add_parser('create-stories', help='Create user stories')
    ...
    if args.command == 'prioritize':
        ...
        rice_scores = None
        if args.rice_scores:
            with open(args.rice_scores) as f:
