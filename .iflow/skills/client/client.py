...
class Client:
    ...
    def __init__(self, repo_root: Path | None = None):
        ...
        self.templates_dir = (
            Path(__file__).parent.parent / ".shared-state" / "templates"
        )
        ...
        try:
            ...
            if self.templates_dir.exists():
                for template_file in self.templates_dir.glob("*.template.md"):
                    target_file = state_dir / template_file.name.replace(
                        ".template.md", ".md"
                    )
                    ...
                    if not target_file.exists():
                        ...
                        self.logger.info(
                            f"Copied template: {template_file.name} -> {target_file.name}"
                        )
                    else:
                        self.logger.debug(
                            f"Template already exists: {target_file.name}"
                        )
            else:
                self.logger.warning(
...
"""
            ...
            for idx, req in enumerate(
                requirements.get("functional_requirements", []), 1
            ):
            ...
            for idx, req in enumerate(
                requirements.get("non_functional_requirements", []), 1
            ):
            ...
            for idx, criterion in enumerate(
                requirements.get("success_criteria", []), 1
            ):
        ...
        try:
            if not pipeline_file.exists():
                return (
                    ...
                    f"Pipeline status file not found: {pipeline_file}",
                )
            ...
            for idx, feature in enumerate(features, 1):
                current = (
                    " (current)"
                    if current_feature and feature == current_feature
                    else ""
                )
                status = (
                    "[x]" if current_feature and feature == current_feature else "[ ]"
                )
            ...
            for file in files:
                ...
                if file_path.exists():
                    code, _, stderr = run_git_command(
                        ["add", str(file_path)], cwd=project_path
                    )
                    if code != 0:
...
"""
            for file in files:
                commit_message += (
                    f"- {project_path}/.iflow/skills/.shared-state/{file}\n"
                )
...
- TDD: compliant"""
    ...
    def run_workflow(
        ...
        if code != 0:
            self.logger.warning(
                "Failed to extract features, continuing with empty feature list"
            )
            features = []
        ...
        if features:
            ...
            if code != 0:
                self.logger.warning(
                    f"Failed to update pipeline status with features: {message}"
                )

        # Step 6: Commit changes
        if self.config.get("auto_commit", True):
            code, message = self.commit_changes(
                project_path, "document project requirements and feature list"
            )
            if code != 0:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Client skill for requirements gathering and project initialization"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pipeline-type",
        type=str,
        choices=["new-project", "new-feature", "fix-bug"],
        help="Type of pipeline",
    )
    parser.add_argument(
        "--iteration-mode",
        type=str,
        choices=["feature", "all-at-once"],
        help="Iteration mode",
    )
    parser.add_argument(
        "--current-feature", type=str, help="Current feature being processed"
    )
    parser.add_argument(
        "--requirements-file", type=str, help="JSON file containing requirements"
    )
    ...
    spec_parser = subparsers.add_parser(
        "create-spec", help="Create project specification"
    )
    spec_parser.add_argument(
        "--requirements-file",
        type=str,
        required=True,
        help="JSON file containing requirements",
    )
    ...
    commit_parser.add_argument(
        "--description", type=str, required=True, help="Description of changes"
    )
