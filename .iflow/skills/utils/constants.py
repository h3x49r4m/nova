...
class SecretPatterns(Enum):
    ...
    DIGITALOCEAN_TOKEN = r"digitalocean[_-]?token\s*[=:]\s*['\"]?[a-f0-9]{64}"
    CLOUDFLARE_API_KEY = r"cloudflare[_-]?api[_-]?key\s*[=:]\s*['\"]?[a-f0-9]{37}"
    CIRCLECI_TOKEN = r"circle[_-]?token\s*[=:]\s*['\"]?[a-f0-9]{40}"
    TRAVIS_CI_TOKEN = r"travis[_-]?token\s*[=:]\s*['\"]?[a-zA-Z0-9_\-\.]{22,}"
    GITHUB_ACTIONS_TOKEN = r"gh[pou]s_[a-zA-Z0-9_]{36,}"
    POSTGRES_PASSWORD = r"postgres[_-]?(password|pwd)\s*[=:]\s*['\"]?[^\s'\"]{8,}"
    MYSQL_PASSWORD = r"mysql[_-]?(password|pwd)\s*[=:]\s*['\"]?[^\s'\"]{8,}"
    SENDGRID_API_KEY = r"sendgrid[_-]?api[_-]?key\s*[=:]\s*['\"]?SG\.[a-zA-Z0-9_\-\.]{68}"
    MAILGUN_API_KEY = r"mailgun[_-]?api[_-]?key\s*[=:]\s*['\"]?[a-f0-9]{32}"
    TWILIO_AUTH_TOKEN = r"twilio[_-]?auth[_-]?token\s*[=:]\s*['\"]?[a-zA-Z0-9]{32}"
    TWILIO_ACCOUNT_SID = r"twilio[_-]?account[_-]?sid\s*[=:]\s*['\"]?AC[a-f0-9]{32}"
    LINKEDIN_CLIENT_SECRET = r"linkedin[_-]?client[_-]?secret\s*[=:]\s*['\"]?[a-zA-Z0-9]{16}"
    LINKEDIN_ACCESS_TOKEN = r"linkedin[_-]?access[_-]?token\s*[=:]\s*['\"]?[a-zA-Z0-9\-]{60,}"
    REDIS_PASSWORD = r"redis[_-]?(password|pwd)\s*[=:]\s*['\"]?[^\s'\"]{8,}"
    MONGODB_URI = r"mongodb(?:\+srv)?://[^\s'\"]+:[^\s'\"]+@"
    ELASTICSEARCH_PASSWORD = r"elasticsearch[_-]?(password|pwd)\s*[=:]\s*['\"]?[^\s'\"]{8,}"
    RABBITMQ_PASSWORD = r"rabbitmq[_-]?(password|pwd)\s*[=:]\s*['\"]?[^\s'\"]{8,}"
    KAFKA_PASSWORD = r"kafka[_-]?(password|pwd)\s*[=:]\s*['\"]?[^\s'\"]{8,}"
    BASE64_ENCODED_SECRET = r"(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)"
