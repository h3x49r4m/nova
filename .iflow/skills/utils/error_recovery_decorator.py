...
class ErrorRecoveryContext:
    ...
    def __init__(self, config: RecoveryConfig, logger: StructuredLogger | None = None):
        """
    ...
    def should_retry(self, error: Exception) -> bool:
        ...
        if isinstance(error, IFlowError):
            return bool((error.category.name == 'TRANSIENT' and self.config.retry_on_transient) or (error.category.name == 'PERMANENT' and self.config.retry_on_permanent))
    ...
    def get_result(self, success: bool, final_value: Any = None, final_error: Exception | None = None) -> RecoveryResult:
        """
