...
class PipelineState:
    ...
    def get_next_ready_stage(self) -> Stage | None:
...
class PipelineOrchestrator:
    ...
    def execute(self) -> tuple[int, str]:
        ...
        try:
            ...
            self.state.completed_at = datetime.now().isoformat()
            self.state.updated_at = datetime.now().isoformat()
            return 1, f"Unexpected error: {e!s}"
    ...
    def _execute_stage(self, stage: Stage) -> tuple[int, str]:
        ...
        try:
            ...
            stage.error = str(e)
            stage.completed_at = datetime.now().isoformat()
            return 1, f"Stage execution error: {e!s}"
    ...
    def _invoke_skill(self, stage: Stage) -> tuple[int, str]:
        ...
        try:
            ...
            return 1, f"Skill invocation error: {e!s}"
...
"""
    ...
    def _role_to_skill(self, role: str) -> str | None:
    ...
    def load_state(self, file_path: Path) -> tuple[int, str]:
        ...
        try:
            ...
            return 1, f"Failed to load pipeline state: {e!s}"
