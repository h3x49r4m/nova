...
class ConfigValidator:
    ...
    def validate_pipeline_config(self, config_path: Path) -> tuple[int, str]:
        ...
        try:
            ...
            return 1, f"Invalid JSON in configuration file: {e!s}"
        except Exception as e:
            return 1, f"Failed to validate configuration: {e!s}"
...
def validate_config_file(config_path: Path, schema_dir: Path | None = None) -> tuple[int, str]:
    """
...
def create_config_validator(schema_dir: Path | None = None) -> ConfigValidator:
