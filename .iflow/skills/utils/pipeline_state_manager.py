...
from .exceptions import BackupError, FileError
...
class PipelineStateManager:
    ...
    def save_state(
        ...
        try:
            ...
            with FileLock(self.state_file, timeout=30):
                # Add metadata
                state["_metadata"] = {
            ...
            return 1, f"Failed to save state: {e!s}"
    ...
    def load_state(self) -> tuple[int, str, dict[str, Any] | None]:
        ...
        try:
            ...
            with FileLock(self.state_file, mode='r', timeout=30):
            ...
            return 1, f"Invalid JSON in state file: {e!s}", None
        except Exception as e:
            return 1, f"Failed to load state: {e!s}", None
    ...
    def delete_state(self, create_backup: bool = True) -> tuple[int, str]:
        ...
        try:
            ...
            return 1, f"Failed to delete state: {e!s}"
    ...
    def restore_backup(self, backup_index: int = 0) -> tuple[int, str]:
        ...
        try:
            ...
            if code == 0:
                ...
                code2, output2, _state = self.load_state()
                if code2 != 0:
            ...
            return 1, f"Failed to restore backup: {e!s}"
    ...
    def list_backups(self) -> tuple[int, str, list[dict[str, Any]]]:
        ...
        try:
            ...
            return 1, f"Failed to list backups: {e!s}", []
    ...
    def _validate_state(self, state: dict[str, Any]) -> tuple[bool, list[str] | None]:
    ...
    def export_state(self, export_path: Path) -> tuple[int, str]:
        ...
        try:
            ...
            return 1, f"Failed to export state: {e!s}"
    ...
    def import_state(self, import_path: Path, replace: bool = False) -> tuple[int, str]:
        ...
        try:
            ...
            if self.state_file.exists() and not replace:
                return 1, "State file already exists. Use replace=True to overwrite."

            # Save imported state
            return self.save_state(imported_state, create_backup=True)

        except json.JSONDecodeError as e:
            return 1, f"Invalid JSON in import file: {e!s}"
        except Exception as e:
            return 1, f"Failed to import state: {e!s}"
