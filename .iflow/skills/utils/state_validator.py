...
class StateValidator:
    ...
    def _validate_schema(self, state: dict[str, Any], state_type: str, result: ValidationResult):
        ...
        try:
            ...
            result.warnings.append(f"Schema validation failed: {e!s}")
    ...
    def _validate_branch_name(self, branch_name: str) -> tuple[bool, str | None]:
        """
    ...
    def _validate_version(self, version: str) -> tuple[bool, str | None]:
        """
    ...
    def validate_state_file(
        ...
        try:
            ...
            return ValidationResult(
                is_valid=False,
                errors=[f"Invalid JSON in state file: {e!s}"],
                warnings=[],
                info=[]
            )
        except OSError as e:
            return ValidationResult(
                is_valid=False,
                errors=[f"Could not read state file: {e!s}"],
                warnings=[],
                info=[]
            )
