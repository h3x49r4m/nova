...
class Workflow:
    ...
    def get_step_by_number(self, step_number: int) -> WorkflowStep | None:
...
class WorkflowParser:
    ...
    def _extract_objective(self, lines: list[str]) -> str:
        ...
        for i, line in enumerate(lines):
            if line.startswith('## Objective') and i + 1 < len(lines):
    ...
    def _extract_output(self, lines: list[str]) -> str | None:
        """Extract output from workflow lines."""
        for i, line in enumerate(lines):
            if line.startswith('## Output') and i + 1 < len(lines):
                return lines[i + 1].strip()
        return None
