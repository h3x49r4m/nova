...
class SkillConfigValidator:
    ...
    def validate(self, config: dict[str, Any]) -> tuple[bool, list[str]]:
        ...
        if not isinstance(config["description"], str) or len(config["description"]) < 10:
            errors.append(
                "Invalid description: must be a string with at least 10 characters"
            )
...
def validate_skill_config(config_path: Path) -> tuple[int, str]:
    ...
    is_valid, errors, _config = validator.validate_file(config_path)
...
def normalize_skill_config_file(input_path: Path, output_path: Path | None = None) -> tuple[int, str]:
    """
    ...
    if not is_valid:
        return 1, "Cannot normalize invalid config:\n" + "\n".join(f"  - {e}" for e in errors)
    ...
    try:
        ...
        return 1, f"Failed to save normalized configuration: {e!s}"
...
def migrate_all_configs(config_dir: Path, dry_run: bool = False) -> tuple[int, str, list[dict[str, Any]]]:
    ...
    for config_file in config_files:
        is_valid, _errors, config = validator.validate_file(config_file)
        ...
        if not is_valid:
            ...
            try:
                ...
                results.append({
                    "file": str(config_file),
                    "status": "error",
                    "message": f"Failed to normalize: {e!s}"
                })
        else:
