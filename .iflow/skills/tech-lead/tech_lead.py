...
class TechLead:
        ...
        try:
            if not spec_file.exists():
                return (
                    ...
                    f"Project spec not found: {spec_file}",
                )
    ...
    def extract_requirements(self, content: str) -> Requirements:
        ...
        for idx, description in fr_matches:
            ...
            requirements["functional"].append(req_item)
        ...
        for idx, description in nfr_matches:
            ...
            if "scal" in desc_lower or "concurrent" in desc_lower:
                requirements["scale"] = ScaleLevel.LARGE
            req_item: RequirementItem = {
                "id": f"NFR{idx}",
                "description": description.strip(),
            }
            requirements["non_functional"].append(req_item)
    ...
    def design_system_architecture(
        ...
        architecture_pattern = self.config.get(
            "architecture_pattern", ArchitecturePattern.MONOLITH
        )
    ...
    def _design_data_flow(self, pattern: str) -> list[dict[str, Any]]:
        ...
        return [
            ...
            {"step": 2, "description": "Gateway authenticates request", "type": "auth"},
            {
                "step": 3,
    ...
    def define_design_patterns(self) -> dict[str, Any]:
        ...
        design_pattern = self.config.get(
            "design_pattern", DesignPattern.CLEAN_ARCHITECTURE
        )
        ...
        if design_pattern == DesignPattern.CLEAN_ARCHITECTURE:
            patterns["layers"] = [
                ...
                },
                {
                    ...
                    "responsibility": "Use cases and business logic",
                },
                {
                    "name": "Domain Layer",
                ...
                },
                {
                    ...
                    "responsibility": "External services and data access",
                },
            ]
...
"""
            ...
            spec_content += "\n## System Architecture\n\n"
            spec_content += (
                f"### Pattern: {architecture.get('pattern', 'monolith').upper()}\n\n"
            )
            spec_content += (
                f"{InputSanitizer.sanitize_html(architecture.get('overview', ''))}\n\n"
            )
            ...
            spec_content += (
                f"- **Containerization:** {deploy.get('containerization', 'N/A')}\n"
            )
            spec_content += (
                f"- **Orchestration:** {deploy.get('orchestration', 'N/A')}\n\n"
            )
            ...
            spec_content += (
                f"**Authentication:** {api_spec.get('authentication', 'JWT')}\n"
            )
            ...
            spec_content += (
                f"**Database:** {database_schema.get('database', 'PostgreSQL')}\n"
            )
            ...
            spec_content += (
                f"- **Token Expiry:** {auth.get('token_expiry', '1 hour')}\n"
            )
            ...
            spec_content += (
                f"- **Permissions:** {', '.join(authz.get('permissions', []))}\n\n"
            )
            ...
            spec_content += (
                f"- **Rate Limiting:** {api_sec.get('rate_limiting', '100 req/min')}\n"
            )
            spec_content += (
                f"- **Input Validation:** {api_sec.get('input_validation', 'Strict')}\n"
            )
            spec_content += (
                f"- **Output Encoding:** {api_sec.get('output_encoding', 'Enabled')}\n"
            )
            spec_content += (
                f"- **CORS Policy:** {api_sec.get('cors_policy', 'Restrictive')}\n"
            )
    ...
    def commit_changes(
        self, project_path: Path, changes_description: str
    ) -> tuple[int, str]:
        """Commit changes with proper metadata."""
        try:
            # Get current branch
            code, branch, _ = run_git_command(
...
- TDD: compliant"""
            ...
            error_msg = f"Git command timed out: {e}"
            self.logger.error(error_msg)
            return ErrorCode.TIMEOUT.value, error_msg
        except GitError as e:
            error_msg = f"Git command failed: {e}"
            self.logger.error(error_msg)
            return e.code.value, error_msg
        except OSError as e:
            error_msg = f"File system error: {e}"
            self.logger.error(error_msg)
            return ErrorCode.FILE_OPERATION_ERROR.value, error_msg
    ...
    def update_pipeline_status(
        self, project_path: Path, phase_name: str, status: str = "completed"
    ) -> tuple[int, str]:
        ...
        try:
            if not pipeline_file.exists():
                return (
                    ErrorCode.FILE_NOT_FOUND.value,
                    f"Pipeline status not found: {pipeline_file}",
                )
            ...
            content = re.sub(r"\*\*Progress:\*\* \d+%", "**Progress:** 40%", content)
    ...
    def run_workflow(self, project_path: Path) -> tuple[int, str]:
        ...
        if self.config.get("auto_commit", True):
            code, message = self.commit_changes(
                project_path, "design system architecture and tech stack"
            )
            if code != 0:
        ...
        code, message = self.update_pipeline_status(
            project_path, "Planning & Design", "completed"
        )
        if code != 0:
        ...
        return (
            0,
            f"Tech lead workflow completed successfully. Designed {architecture.get('pattern', 'monolith')} architecture with {len(tech_stack)} technology stack components, {len(api_spec.get('endpoints', []))} API endpoints, and {len(database_schema.get('tables', []))} database tables.",
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Tech Lead skill for architecture design and technical strategy"
    )
    parser.add_argument(
        "--project-path", type=str, help="Path to the project directory"
    )
    ...
    workflow_parser = subparsers.add_parser(
        "run", help="Run complete tech lead workflow"
    )
    workflow_parser.add_argument(
        "--project-path", type=str, required=True, help="Path to the project directory"
    )
    ...
    if args.command == "design-architecture":
        ...
        for category, stack in tech_stack.items():
            print(
                f"  {category.title()}: {stack.get('framework' if category != 'database' else 'primary', 'N/A')}"
            )
        ...
        print(
            f"Primary Pattern: {design_patterns.get('primary_pattern', 'clean_architecture').upper()}"
        )
